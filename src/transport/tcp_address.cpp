...
slk::tcp_address_t::tcp_address_t () : _has_src_addr (false)
{
    memset (&_address, 0, sizeof (_address));
    memset (&_source_address, 0, sizeof (_source_address));
}

slk::tcp_address_t::tcp_address_t (const sockaddr *sa_, socklen_t sa_len_) :
    _has_src_addr (false)
{
    slk_assert (sa_ && sa_len_ > 0);
...
int slk::tcp_address_t::resolve (const char *name_, bool local_, bool ipv6_)
{
                                        ...
                                        const char (&ipv6_suffix_)[N2])
{
    ...
    slk_assert (res > 0 && res < (int) (max_port_str_length + 1));
    pos += res;
    return std::string (buf, pos - buf);
}

int slk::tcp_address_t::to_string (std::string &addr_) const
{
...
const sockaddr *slk::tcp_address_t::addr () const
{
    return _address.as_sockaddr ();
}

socklen_t slk::tcp_address_t::addrlen () const
{
    return _address.sockaddr_len ();
}

const sockaddr *slk::tcp_address_t::src_addr () const
{
    return _source_address.as_sockaddr ();
}

socklen_t slk::tcp_address_t::src_addrlen () const
{
    return _source_address.sockaddr_len ();
}

bool slk::tcp_address_t::has_src_addr () const
{
    return _has_src_addr;
}

#if defined SL_HAVE_WINDOWS
unsigned short slk::tcp_address_t::family () const
#else
sa_family_t slk::tcp_address_t::family () const
#endif
{
    return _address.family ();
}

slk::tcp_address_mask_t::tcp_address_mask_t () : _address_mask (-1)
{
    memset (&_network_address, 0, sizeof (_network_address));
}

int slk::tcp_address_mask_t::resolve (const char *name_, bool ipv6_)
{
...
bool slk::tcp_address_mask_t::match_address (const struct sockaddr *ss_,
                                             const socklen_t ss_len_) const
{
    slk_assert (_address_mask != -1 && ss_ != NULL
                && ss_len_
    ...
    if (_address_mask > 0) {
        ...
        if (ss_->sa_family == AF_INET6) {
            slk_assert (ss_len_ == sizeof (struct sockaddr_in6));
            ...
            slk_assert (ss_len_ == sizeof (struct sockaddr_in));
            their_bytes = reinterpret_cast<const uint8_t *> (&(
