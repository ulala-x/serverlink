...
int slk::ip_addr_t::family () const
{
    return generic.sa_family;
}

bool slk::ip_addr_t::is_multicast () const
{
    if (family () == AF_INET) {
...
uint16_t slk::ip_addr_t::port () const
{
    if (family () == AF_INET6) {
...
const struct sockaddr *slk::ip_addr_t::as_sockaddr () const
{
    return &generic;
}

slk::slk_socklen_t slk::ip_addr_t::sockaddr_len () const
{
    return static_cast<slk_socklen_t> (family () == AF_INET6 ? sizeof (ipv6)
                                                             : sizeof (ipv4));
}

void slk::ip_addr_t::set_port (uint16_t port_)
{
    if (family () == AF_INET6) {
...
slk::ip_addr_t slk::ip_addr_t::any (int family_)
{
    ip_addr_t addr;
...
slk::ip_resolver_options_t::ip_resolver_options_t () :
...
slk::ip_resolver_options_t &
slk::ip_resolver_options_t::bindable (bool bindable_)
{
...
slk::ip_resolver_options_t &
slk::ip_resolver_options_t::allow_nic_name (bool allow_)
{
...
slk::ip_resolver_options_t &slk::ip_resolver_options_t::ipv6 (bool ipv6_)
{
    _ipv6_wanted = ipv6_;
...
slk::ip_resolver_options_t &
slk::ip_resolver_options_t::expect_port (bool expect_)
{
...
slk::ip_resolver_options_t &slk::ip_resolver_options_t::allow_dns (bool allow_)
{
    _dns_allowed = allow_;
...
slk::ip_resolver_options_t &slk::ip_resolver_options_t::allow_path (bool allow_)
{
...
bool slk::ip_resolver_options_t::bindable ()
{
    return _bindable_wanted;
}

bool slk::ip_resolver_options_t::allow_nic_name ()
{
    return _nic_name_allowed;
}

bool slk::ip_resolver_options_t::ipv6 ()
{
    return _ipv6_wanted;
}

bool slk::ip_resolver_options_t::expect_port ()
{
    return _port_expected;
}

bool slk::ip_resolver_options_t::allow_dns ()
{
    return _dns_allowed;
}

bool slk::ip_resolver_options_t::allow_path ()
{
    return _path_allowed;
}

slk::ip_resolver_t::ip_resolver_t (ip_resolver_options_t opts_) :
    _options (opts_)
{
}

int slk::ip_resolver_t::resolve (ip_addr_t *ip_addr_, const char *name_)
{
    std::string addr;
    uint16_t port;
...
int slk::ip_resolver_t::resolve_getaddrinfo (ip_addr_t *ip_addr_,
                                             const char *addr_)
{
#if defined SL_HAVE_OPENVMS && defined __ia64
    ...
    slk_assert (res != NULL);
    slk_assert (static_cast<size_t> (res->ai_addrlen) <= sizeof (*ip_addr_));
...
#ifdef SL_HAVE_SOLARIS
#include <sys/sockio.h>

//  On Solaris platform, network interface name can be queried by ioctl.
int slk::ip_resolver_t::resolve_nic_name (ip_addr_t *ip_addr_, const char *nic_)
{
    //  Create a socket.
...
#elif defined SL_HAVE_AIX || defined SL_HAVE_HPUX                            \
  || defined SL_HAVE_ANDROID || defined SL_HAVE_VXWORKS
#include <sys/ioctl.h>
#ifdef SL_HAVE_VXWORKS
#include <ioLib.h>
#endif
...
int slk::ip_resolver_t::resolve_nic_name (ip_addr_t *ip_addr_, const char *nic_)
{
#if defined SL_HAVE_AIX || defined SL_HAVE_HPUX
...
#elif ((defined SL_HAVE_LINUX || defined SL_HAVE_FREEBSD                     \
        || defined SL_HAVE_OSX || defined SL_HAVE_OPENBSD                    \
        || defined SL_HAVE_QNXNTO || defined SL_HAVE_NETBSD                  \
        || defined SL_HAVE_DRAGONFLY || defined SL_HAVE_GNU)                 \
       && defined SL_HAVE_IFADDRS)
...
#elif (defined SL_HAVE_WINDOWS)

#include <netioapi.h>

int slk::ip_resolver_t::get_interface_name (unsigned long index_,
                                            char **dest_) const
{
#ifdef SL_HAVE_WINDOWS_UWP
...
#if _WIN32_WINNT > _WIN32_WINNT_WINXP && !defined SL_HAVE_WINDOWS_UWP
...
int slk::ip_resolver_t::wchar_to_utf8 (const WCHAR *src_, char **dest_) const
{
    int rc;
...
int slk::ip_resolver_t::do_getaddrinfo (const char *node_,
                                        const char *service_,
...
void slk::ip_resolver_t::do_freeaddrinfo (struct addrinfo *res_)
{
    freeaddrinfo (res_);
}


unsigned int slk::ip_resolver_t::do_if_nametoindex (const char *ifname_)
{
