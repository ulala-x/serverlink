...
int slk::tune_tcp_socket (fd_t s_)
{
    //  Disable Nagle's algorithm. We are doing data batching on SL level,
...
int slk::set_tcp_send_buffer (fd_t sockfd_, int bufsize_)
{
    const int rc =
...
int slk::set_tcp_receive_buffer (fd_t sockfd_, int bufsize_)
{
    const int rc =
...
int slk::tune_tcp_keepalives (fd_t s_,
                              int keepalive_,
                              int keepalive_cnt_,
                              int keepalive_idle_,
                              int keepalive_intvl_)
{
    ...
    SL_UNUSED (keepalive_);
    SL_UNUSED (keepalive_cnt_);
    SL_UNUSED (keepalive_idle_);
    SL_UNUSED (keepalive_intvl_);
...
#endif // SL_HAVE_TCP_KEEPCNT

#ifdef SL_HAVE_TCP_KEEPIDLE
...
#else // SL_HAVE_TCP_KEEPIDLE
#ifdef SL_HAVE_TCP_KEEPALIVE
        if (keepalive_idle_ != -1) {
...
#endif // SL_HAVE_TCP_KEEPALIVE
#endif // SL_HAVE_TCP_KEEPIDLE

#ifdef SL_HAVE_TCP_KEEPINTVL
...
#endif // SL_HAVE_TCP_KEEPINTVL
    }
#endif // SL_HAVE_SO_KEEPALIVE
#endif // SL_HAVE_WINDOWS

    return 0;
}

int slk::tune_tcp_maxrt (fd_t sockfd_, int timeout_)
{
    if (timeout_ <= 0)
        return 0;
...
#if defined(SL_HAVE_WINDOWS) && defined(TCP_MAXRT)
    ...
    assert_success_or_recoverable (sockfd_, rc);
    return rc;
// FIXME: should be SL_HAVE_TCP_USER_TIMEOUT
...
int slk::tcp_write (fd_t s_, const void *data_, size_t size_)
{
#ifdef SL_HAVE_WINDOWS
...
int slk::tcp_read (fd_t s_, void *data_, size_t size_)
{
#ifdef SL_HAVE_WINDOWS
...
void slk::tcp_tune_loopback_fast_path (const fd_t socket_)
{
#if defined SL_HAVE_WINDOWS && defined SIO_LOOPBACK_FAST_PATH
...
void slk::tune_tcp_busy_poll (fd_t socket_, int busy_poll_)
{
#if defined(SL_HAVE_BUSY_POLL)
    if (busy_poll_ > 0) {
        const int rc =
...
#else
    SL_UNUSED (socket_);
    SL_UNUSED (busy_poll_);
#endif
}

slk::fd_t slk::tcp_open_socket (const char *address_,
                                const slk::options_t &options_,
                                bool local_,
                                bool fallback_to_ipv4_,
                                slk::tcp_address_t *out_tcp_addr_)
{
