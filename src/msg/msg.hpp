...
//  slk_free_fn defined in serverlink.h.
extern "C" {
...
class msg_t
{
  public:
    ...
    //  are at least 2 references to it) refcnt member contains number of
    //  references.
    struct content_t
    {
        void *data;
  ...
  private:
    ...
    struct long_group_t
    {
        char group[SL_GROUP_MAX_LENGTH + 1];
...
inline int close_and_return (slk::msg_t msg_[], int count_, int echo_)
{
