...
#ifndef SERVERLINK_SOCKET_POLLER_HPP
#define SERVERLINK_SOCKET_POLLER_HPP

#include "../io/poller.hpp"

#if defined SL_POLL_BASED_ON_POLL && !defined SL_HAVE_WINDOWS
#include <poll.h>
#endif
...
class socket_poller_t
{
  ...
  private:
    ...
    static void zero_trail_events (slk::socket_poller_t::event_t *events_,
                                   int n_events_,
                                   int found_);
#if defined SL_POLL_BASED_ON_POLL
    int check_events (slk::socket_poller_t::event_t *events_, int n_events_);
#elif defined SL_POLL_BASED_ON_SELECT
    int check_events (slk::socket_poller_t::event_t *events_,
                      int n_events_,
                      fd_set &inset_,
                      fd_set &outset_,
                      fd_set &errset_);
#endif
    static int adjust_timeout (slk::clock_t &clock_,
                               long timeout_,
                               uint64_t &now_,
                               uint64_t &end_,
                               bool &first_pass_);
    ...
    socket_poller_t (const socket_poller_t &) = delete;
    socket_poller_t &operator= (const socket_poller_t &) = delete;
};
}
