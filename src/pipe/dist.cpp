...
void slk::dist_t::attach (pipe_t *pipe_)
{
...
bool slk::dist_t::has_pipe (pipe_t *pipe_)
{
    const pipes_t::size_type claimed_index = _pipes.index (pipe_);
...
void slk::dist_t::match (pipe_t *pipe_)
{
...
void slk::dist_t::unmatch ()
{
    _matching = 0;
}

void slk::dist_t::pipe_terminated (pipe_t *pipe_)
{
...
int slk::dist_t::send_to_all (msg_t *msg_)
{
    _matching = _active;
    return send_to_matching (msg_);
}

int slk::dist_t::send_to_matching (msg_t *msg_)
{
...
size_t slk::dist_t::get_pipe_count () const
{
    return _pipes.size ();
}

slk::pipe_t *slk::dist_t::get_pipe (size_t index_) const
{
    if (index_ >= _pipes.size ())
        return NULL;
    return const_cast<pipe_t *> (_pipes[index_]);
}
