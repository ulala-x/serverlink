...
slk::trie_t::trie_t () : _refcnt (0), _min (0), _count (0), _live_nodes (0)
{
}
...
bool slk::trie_t::add (unsigned char *prefix_, size_t size_)
{
...
bool slk::trie_t::rm (unsigned char *prefix_, size_t size_)
{
    if (!size_) {
        if (!_refcnt)
            return false;
    ...
    if (next_node->is_redundant ()) {
        ...
        if (_count == 1) {
            ...
            if (_live_nodes == 1) {
                ...
                slk_assert (new_min != _min);

                trie_t **old_table = _next.table;
                slk_assert (new_min > _min);
                slk_assert (_count > new_min - _min);
...
bool slk::trie_t::check (const unsigned char *data_, size_t size_) const
{
...
void slk::trie_t::apply_helper (unsigned char **buff_,
                                size_t buffsize_,
                                size_t maxbuffsize_,
...
bool slk::trie_t::is_redundant () const
{
