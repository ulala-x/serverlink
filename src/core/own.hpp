...
#ifndef SL_OWN_HPP_INCLUDED
#define SL_OWN_HPP_INCLUDED
...
class own_t : public object_t
{
  public:
    ...
    // thread outside of ServerLink infrastructure
    own_t (slk::ctx_t *parent_, uint32_t tid_);

    // The object is living within I/O thread
    own_t (slk::io_thread_t *io_thread_, const options_t &options_);
    ...
    // register_term_acks functions. When event occurs, call
    // unregister_term_ack. When number of pending acks reaches zero
  ...
  private:
    ...
    SL_NON_COPYABLE_NOR_MOVABLE (own_t)
};
}
