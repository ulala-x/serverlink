...
{
    _ctx->unregister_endpoints (socket_);
}
...
{
    _ctx->connect_pending (addr_, bind_socket_);
}
...
void slk::object_t::process_reAP (int)
{
    slk_assert (false);
}

void slk::object_t::process_reconnect (i_engine *)
{
    slk_assert (false);
}
...
void slk::object_t::process_reap (socket_base_t *)
{
    slk_assert (false);
}
...
void slk::object_t::send_stop ()
{
    ...
    _ctx->send_command (ctx_t::reaper_tid, cmd);
}
...
void slk::object_t::send_reAP (monitor_t *destination_, int interval_)
{
    SL_UNUSED (destination_);
    SL_UNUSED (interval_);
    slk_assert (false);
}

void slk::object_t::send_reconnect (session_base_t *destination_,
                                    i_engine *engine_)
{
    command_t cmd;
    cmd.destination = destination_;
    cmd.type = command_t::reconnect;
    cmd.args.reconnect.engine = engine_;
    _ctx->send_command (destination_->get_tid (), cmd);
}
...
void slk::object_t::send_pipe_stats_publish (own_t *destination_,
...
void slk::object_t::send_reap (socket_base_t *socket_)
{
    command_t cmd;
    cmd.destination = _ctx->get_reaper ();
...
void slk::object_t::send_inproc_connected (socket_base_t *socket_)
{
    command_t cmd;
    cmd.destination = socket_;
    cmd.type = command_t::inproc_connected;
    _ctx->send_command (socket_->get_tid (), cmd);
}