...
#define TCP_ADDR "tcp://127.0.0.1:17777"
...
void* run_server(void* arg) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (type == ZMQ_ROUTER) {
            ...
            if (id_len < 0) { test_failed = 1; break; }
        }
        int data_len = zmq_recv(sock, buffer, global_msg_size + 1024, 0);
        if (data_len < 0) { test_failed = 1; break; }
...
int main(int argc, char** argv) {
    ...
    if (c_type == ZMQ_ROUTER) zmq_setsockopt(client_sock, ZMQ_ROUTING_ID, "CLIENT", 6);
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (mode == 1 && c_type != ZMQ_PUB) zmq_recv(client_sock, data, global_msg_size, 0);
    }
