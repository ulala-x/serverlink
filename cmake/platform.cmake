...
# TEMPORARY: IOCP disabled pending full integration with existing architecture
# Enable by setting SL_USE_IOCP=ON in CMake options
option(SL_USE_IOCP "Enable IOCP (Windows I/O Completion Ports)" OFF)
if(WIN32 AND SL_USE_IOCP)
    check_symbol_exists(CreateIoCompletionPort "windows.h" HAVE_IOCP)
    if(HAVE_IOCP)
        set(SL_HAVE_IOCP 1)
        message(STATUS "IOCP (I/O Completion Ports) support ENABLED")
    else()
        set(SL_HAVE_IOCP 0)
    endif()
else()
    set(SL_HAVE_IOCP 0)
    if(WIN32)
        message(STATUS "IOCP disabled, using select() on Windows")
    endif()
endif()
...
# Priority order: IOCP (Windows) > wepoll (Windows) > epoll (Linux) > kqueue (BSD/macOS) > select (fallback)
if(SL_HAVE_IOCP)
    set(SL_POLLER_NAME "iocp")
...
message(STATUS "  IOCP:            ${SL_HAVE_IOCP}")
